...
export const MidiEditorApp: React.FC<MidiEditorAppProps> = ({ onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900">
      {/* Header */}
      <div className="h-14 bg-black/30 border-b border-zinc-700 flex items-center justify-between px-4 flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-white">MIDI Editor</h1>
          {editingSequence && (
            <input
              type="text"
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 bg-zinc-800 rounded px-2 py-1">
            ...
            <input
              ...
              className="w-14 bg-transparent text-white text-center text-sm outline-none"
            />
            <input
              type="range"
              min="20"
          ...
          <button
            ...
            className={`p-2 rounded text-white ${isPlaying ? 'bg-red-600 hover:bg-red-500' : 'bg-green-600 hover:bg-green-500'}`}
          ...
          <button
            ...
            disabled={!editingSequence}
            className="px-3 py-1.5 bg-zinc-700 hover:bg-zinc-600 disabled:opacity-50 rounded text-sm text-white"
          >
            Export .mid
          </button>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-64 bg-black/20 border-r border-zinc-700 flex flex-col flex-shrink-0">
          <div className="p-3 border-b border-zinc-700">
            <div className="flex gap-2 mb-2">
              <button
                onClick={handleNewMidi}
          ...
          <div className="flex-1 overflow-y-auto p-2">
            <div className="text-xs text-zinc-500 uppercase mb-2">MIDI Files</div>
            {midiFiles.length === 0 ? (
              ...
              midiFiles.map(midi => (
                <div
                  key={midi.id}
                  className={`p-2 rounded mb-1 cursor-pointer flex items-center justify-between group ${
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {/* Toolbar */}
          <div className="h-10 bg-zinc-800 border-b border-zinc-700 flex items-center px-3 gap-2 flex-shrink-0">
            <div className="flex gap-1 bg-zinc-900 rounded p-1">
              <button
                onClick={() => setTool('select')}
                className={`p-1.5 rounded text-sm ${tool === 'select' ? 'bg-indigo-600 text-white' : 'text-zinc-400 hover:text-white'}`}
                title="Select (V)"
              >
              ...
              <button
                ...
                className={`p-1.5 rounded text-sm ${tool === 'pencil' ? 'bg-indigo-600 text-white' : 'text-zinc-400 hover:text-white'}`}
                title="Pencil (P)"
              >
              ...
              <button
                ...
                className={`p-1.5 rounded text-sm ${tool === 'eraser' ? 'bg-indigo-600 text-white' : 'text-zinc-400 hover:text-white'}`}
                title="Eraser (E)"
              >
            ...
            <button
              ...
              className="px-2 py-1 bg-red-600/50 hover:bg-red-600 disabled:opacity-30 rounded text-xs text-white"
            >
              Delete Selected
            </button>
          ...
          {editingSequence ? (
            <div className="flex-1 flex overflow-hidden">
              {/* Piano keys */}
              <div className="w-12 bg-zinc-900 flex-shrink-0 overflow-y-auto scrollbar-thin">
                {renderPianoKeys()}
              </div>
            ...
            <div className="flex-1 flex items-center justify-center text-zinc-500">
              <div className="text-center">
                ...
                <div className="text-xl mb-2">No MIDI selected</div>
                <div className="text-sm">Create a new MIDI or select one from the list</div>
              </div>
            </div>
          )}
        </div>
